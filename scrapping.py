...
ncMain = appContent.find('div', class_='nc-main nc-main-v2')
gsc_row = ncMain.find('div', class_='gsc_row')
listitem = gsc_row.find('div', class_='listitems gsc_row')
car_divs = listitem.find_all('div', class_='gsc_col-md-12 gsc_col-sm-12 gsc_col-xs-12 append_list')
        ...
        if gsc_row:
            ...
            if qccontent_div:
                ...
                if stand_out_features_div:
                    ...
                    stand_out_features_string = ",".join(stand_out_features)
                ...
                if variants_table:
                   ...
                   data['Variants'] = ",".join(variants_text) if variants_text else 'No variants found'
                else:
...
for car_div in car_divs:
    ...
    anchor_tag = car_div.find('section', class_='card card_new shadowWPadding overviewTop')
    if anchor_tag:
        for car_div in car_divs:
        # Extract link and title
             anchor_tag = car_div.find('a')
             if anchor_tag:
                ...
                if price_span:
                    price_range = price_span.text.strip().replace('*', '').replace('Get On-Road PriceEx-Showroom Price in New Delhi', '').strip()
                    print("Link:", link)

                    if any(car['Car Name'] == title for car in car_data):
                        print(f"Title '{title}' already exists. Stopping extraction.")
                        break
...
df.to_csv("carDekhoNewCars.csv", index=False)