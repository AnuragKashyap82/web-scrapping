...
def process_year_data(make, model, year, trim):
    ...
    # Find the form with id 'vehicleForm'
    form = soup.find('form', id='vehicleForm')
    if not form:
        print("The form with id 'vehicleForm' was not found.")
        return
    ...
    form_data = {
        'feature': 'used',
        'customer_type': 'dealer',  # Example selection
        'category': 'Car',  # Car
        'make': make,
        'model': model,
        'year': year,
...
# Initial API URL
api_url = "https://orangebookvalue.com/mmyt?&category_id=1&api_version=3"
...
if response.status_code != 200:
    ...
    # Extract the names from the 'data' array
    makes = [item['name'] for item in response_data['data'] if item['selectable']]

    # Loop through each make and call the corresponding API to get models
    for make in makes:
        # Construct the API URL to get models for each make
        models_url = f"https://orangebookvalue.com/mmyt?&category_id=1&make={make}&api_version=3"
        ...
        if models_response.status_code != 200:
            ...
            for model in models[:1]:
                ...
                final_url = f"https://orangebookvalue.com/mmyt?&category_id=1&make={make}&model={model}&api_version=3"
                ...
                if final_response.status_code != 200:
                    ...
                    for year in data_array:
                        year_api_url = f"https://orangebookvalue.com/mmyt?&category_id=1&make={make}&model={model}&year={year}&check_obv=1&api_version=3"
...
df.to_csv("pricerange.csv", index=False)                                 